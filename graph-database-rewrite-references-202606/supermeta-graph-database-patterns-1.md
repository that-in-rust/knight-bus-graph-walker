# Supermeta Graph Database Patterns 1
...
Worker: 1
Exclusive write target: `graph-database-rewrite-references-202606/supermeta-graph-database-patterns-1.md`
...
## Batch 04 Pointer: Adjacent Graph Stores And Pushdown Plans

See `supplemental-gap-closure-batch-04.md` for direct-source evidence from
HugeGraph, Blazegraph, RDF4J, IndraDB, NebulaGraph, and SurrealDB. This batch
adds six patterns that matter for a Neo4j-in-Rust rewrite:

- split physical graph table families before optimizing algorithms;
- model access paths as predicate binding plus index order plus residual filter;
- keep storage/inference compatibility behind closeable cursor boundaries;
- use object-safe Rust graph transaction traits so memory, record, and snapshot
  backends can share tests;
- compile filters and projections into storage-local mini-plans with residual
  execution when pushdown is unsafe;
- treat fresh edge traversal and OLAP deltas as typed directional key ranges
  rather than in-memory object walks.

The same batch also resolves duplicate reference-shelf rows: duplicate upstream
clones should not remain open research gaps when a primary clone is already
direct-source cited. Dgraph was left open at the end of Batch 04 because
posting lists, Badger/Raft storage, schema/index planning, and GraphQL+-style
query execution needed a dedicated direct-source pass.

## Batch 05 Pointer: Dgraph Posting Lists, DQL, And Raft WAL

See `supplemental-gap-closure-batch-05.md` for direct-source evidence from
`gitrefrepo/dgraph-src`. This batch closes the last known primary
`neo4j_gds_compat` gap in the current ledger and adds these patterns:

- predicate-keyed posting lists with immutable base state plus mutable deltas;
- rollup that splits large lists into durable multipart keys;
- delta-only mutation paths that avoid reading base lists when only writing;
- compressed UID/posting-list physical operators for filtered access paths;
- durable key families for data, reverse, index, count, schema, and split
  records;
- schema parsing that produces executable index/rebuild contracts;
- recursive query plans built from source UID lists, UID matrices, value
  matrices, facet matrices, and child subgraphs;
- explicit edge/frontier limits for recurse and shortest-path execution;
- background operation conflict rules for rollup, restore, backup, indexing,
  snapshots, and predicate moves;
- fixed-layout WAL metadata, zeroed log slots, rotation, retry backpressure, and
  timestamp-aware cache validity.

The key architectural lesson is not "copy Dgraph." The lesson is that CSR alone
is too narrow a vocabulary for a Neo4j-compatible low-RAM rewrite. Flat dual CSR
should remain the static OLAP oracle and global-stream primitive, while typed
segmented posting-list sidecars, WAL-derived delta overlays, and schema/index
contracts should be considered before defaulting to a full Tilehouse cell
architecture.
...
- CodeGraphContext indexing of `gitrefrepo/dgraph-src` was interrupted after a
  long silent run, but the resulting database was readable and reported 480
  files, 5,998 functions, 54 interfaces, 585 structs, and 8 modules. Direct
  source reads remain the authority for Batch 05 claims.
