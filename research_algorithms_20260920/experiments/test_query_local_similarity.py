...
class QueryLocalSimilarityTests(unittest.TestCase):
    ...
    def test_releases_previous_plan(self):
        module = self.load_required_local_module()
        targets = [(0, {0, 1, 2, 77}), (1, {4, 5})]
        targets += [(node, {0, 1, 3} if node % 2 == 0 else {0, 2, 3}) for node in range(2, 8)]
        source = module.prepare_skeleton_source_view(prepare_large_group_source(targets, 4, query_limit=10))
        query = {0, 1, 2, 100, 101, 102, 103, 104}
        original, observed = module.build_skeleton_threshold_plan, []

        def inspect_current_plan_lifetime(index, core):
            previous = sys._getframe(1).f_locals.get("plan")
            current = original(index, core)
            observed.append(len(current["nodes"])+(len(previous["nodes"]) if previous is not None else 0))
            return current

        with patch.object(module, "build_skeleton_threshold_plan", inspect_current_plan_lifetime):
            actual, stats = module.run_skeleton_similarity_query(source, query, None, 1)
        self.assertEqual(actual, compute_finite_similarity_oracle(targets, query, None, 1))
        self.assertGreater(len(observed), 1)
        self.assertEqual(max(observed), stats["plan_records_peak"])
