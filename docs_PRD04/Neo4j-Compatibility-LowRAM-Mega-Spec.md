...
**Architecture decision atlas:** `docs_PRD04/reference-learning/lowram-architecture-corpus/LowRAM-Algorithm-Architecture-Decision-Atlas.md`
...
The architecture decision corpus independently reconciles every frozen file in `docs_PRD03` through `docs_PRD06`:

| Document lane | Files | Semantic reads | Structured queries | Generated/superseded/binary classification | Dossier |
|---|---:|---:|---:|---:|---|
| PRD03 | 222 | 37 | 84 | 101 | `reference-learning/lowram-architecture-corpus/agent-04-prd03-architecture.md` |
| PRD04 | 68 | 56 | 6 | 6 | `reference-learning/lowram-architecture-corpus/agent-05-prd04-architecture.md` |
| PRD05 and PRD06 | 94 | 93 | 1 | 0 | `reference-learning/lowram-architecture-corpus/agent-06-prd05-prd06-architecture.md` |
| **Total** | **384** | **186** | **91** | **107** | `scripts/validate_lowram_document_evidence.py` passes |

The status table records method, not truth weight. Generated GDS dossiers and tool outputs are structurally classified or queried rather than treated as independent corroboration. The 27-option normative architecture registry is validated by `scripts/validate_lowram_architecture_atlas.py`. Evidence anchors: PRD03 artifact/spill/oracle analysis `A04-000021`, `A04-000025`; binding PRD04 founder/algorithm decisions `A05-000009`, `A05-000014`; PRD06 CSR, WCC, PageRank, and oracle patterns `A06-000026`, `A06-000018`, `A06-000061`, `A06-000086`.
...
## 1.14 Architecture Registry And Selection Requirements

The normative option catalog is `reference-learning/lowram-architecture-corpus/LowRAM-Algorithm-Architecture-Decision-Atlas.md`. Its 27 option IDs are the permitted starting set, not a promise to implement all nine families immediately.

| Family | Exact fit default | Exact spill default | Retained experiment |
|---|---|---|---|
| Paths/BFS | `ARCH-PATH-001` | `ARCH-PATH-002` | `ARCH-PATH-003` |
| WCC | `ARCH-WCC-001` | `ARCH-WCC-002` | `ARCH-WCC-003` |
| PageRank | `ARCH-PAGERANK-001` | `ARCH-PAGERANK-002` | `ARCH-PAGERANK-003` |
| NodeSimilarity | `ARCH-NODESIM-001` | `ARCH-NODESIM-002` | `ARCH-NODESIM-003` |
| kNN | `ARCH-KNN-001` | `ARCH-KNN-002` | `ARCH-KNN-003` |
| Louvain | `ARCH-LOUVAIN-001` | `ARCH-LOUVAIN-002` | `ARCH-LOUVAIN-003` |
| Leiden | `ARCH-LEIDEN-001` | `ARCH-LEIDEN-002` | `ARCH-LEIDEN-003` |
| Triangles | `ARCH-TRIANGLE-001` | `ARCH-TRIANGLE-002` | `ARCH-TRIANGLE-003` |
| FastRP | `ARCH-FASTRP-001` | `ARCH-FASTRP-002` | `ARCH-FASTRP-003` |

Evidence: algorithm-specific state `A02-007004`, `A02-007156`, `A02-006978`, `A02-007087`, `A02-007056`, `A02-006942`, `A02-006930`, `A02-007127`, `A02-006782`; cross-PRD portfolio decisions `A04-000021`, `A05-000008`, `A05-000014`, `A06-000041`.

### REQ-ARCH-001.0: Bind every run to a registry option

**WHEN** a workload is estimated, admitted, executed, or replayed
**THEN** its canonical plan SHALL name an architecture option ID, option version, representation/view versions, estimator version, and selector version
**AND** the receipt SHALL persist all of those identities
**AND** an unknown, withdrawn, or incompatible option version SHALL be refused before material allocation.

### REQ-ARCH-002.0: Keep a versioned representation registry

**WHEN** a profile needs forward CSR, reverse CSC, edge blocks, destination tiles, degree-oriented adjacency, hub bitmaps, incidence postings, quotient levels, or embedding stripes
**THEN** the representation registry SHALL define logical semantics, physical widths/order/compression, content-addressed build key, validation, byte model, decoder scratch, and eligible consumers
**AND** SHALL report first-build, repeat-run, and amortized preparation separately
**AND** a representation without a validated byte and semantic contract SHALL remain unavailable.

### REQ-ARCH-003.0: Select deterministically from complete inputs

**WHEN** the selector receives identical artifact identity/statistics, workload profile/configuration, allowed exactness/spill policy, resource contract, engine build, and calibration version
**THEN** it SHALL select the same option and canonical plan hash
**AND** SHALL explain rejected alternatives by failed constraint
**AND** missing graph-shape statistics required by an option SHALL cause a conservative alternate or refusal, not optimistic selection.

### REQ-ARCH-004.0: Preserve semantics across physical fallback

**WHEN** an exact fit option is ineligible but its exact spill option is eligible
**THEN** the selector MAY choose the spill option before execution
**AND** SHALL preserve the profile's declared equality relation, tie policy, numeric mode, and output contract
**AND** runtime pressure SHALL NOT silently switch exactness, representation semantics, or approximation after admission
**AND** loss of the admitted spill invariant SHALL cancel or fail the run.

### REQ-ARCH-005.0: Charge irreducible output and mapped residency

**WHEN** an option's output lower bound is `n`, `n*k`, `n*d`, path-enumeration-dependent, or candidate-dependent
**THEN** admission SHALL charge the requested stream buffer, immutable output artifact, or temporary output lower bound explicitly
**AND** SHALL apply a declared policy to mapped clean/dirty pages, page cache, decoder windows, and supervisor overhead
**AND** work whose output/residency policy cannot be bounded SHALL be refused even when core algorithm state fits.

### REQ-ARCH-006.0: Prove fit spill and refusal per family

**WHEN** an algorithm family is advertised
**THEN** release qualification SHALL force its exact fit option, exact spill option, and refusal path on tiny, skewed, and production-shaped artifacts
**AND** SHALL compare their declared equality relation and receipt fields
**AND** a family lacking any one of those executable paths SHALL remain experimental or unavailable.

### REQ-ARCH-007.0: Limit persistent format proliferation

**WHEN** a new derived view is proposed
**THEN** its decision record SHALL identify users, measured build peak, disk bytes, invalidation/freshness rule, expected reuse, first/repeat/amortized time, and a simpler control
**AND** SHALL fit the artifact/profile format-count budget
**AND** a view without measured repeated value SHALL remain per-run, experimental, or rejected rather than becoming permanent compatibility surface.

### REQ-ARCH-008.0: Define family-specific spill invariants and receipts

**WHEN** an exact spill option is implemented
**THEN** its profile SHALL define the family-specific bounded unit, pass/order rule, temporary-space formula, restart boundary, cleanup rule, progress counters, and terminal receipt extensions
**AND** generic “partitioned” or “disk-backed” language without those invariants SHALL fail specification lint
**AND** an oversized recursive partition with no alternate SHALL refuse rather than allocate beyond budget.

### REQ-ARCH-009.0: Gate algorithm breadth on founder evidence

**WHEN** implementation work is proposed beyond bounded paths, WCC, and the first PageRank iterative proof
**THEN** roadmap approval SHALL cite a named buyer workflow, production-shaped artifact, oracle, resource failure, budget owner, paid next step, and the selected atlas options
**AND** SHALL compare the work against improving the existing proof slice
**AND** absent evidence SHALL keep the algorithm architecture-ready but unimplemented.

## 1.15 Storage Layout Requirements
...
## 1.17 Security and Operational Requirements
...
## 1.18 Product-Learning Requirements
...
| TEST-ARCH-001 | ARCH-001..003 | Registry schema + determinism | All 27 atlas options; repeated selector inputs; missing statistics | Every plan binds known versions/views; identical complete inputs select identical hashes; missing required statistics take a conservative alternate or refuse. |
| TEST-ARCH-002 | ARCH-004..006 | Cross-plan differential + forced resource modes | One tiny, skewed, and production-shaped fixture per advertised family | Forced fit and spill satisfy the same declared equality relation; forced refusal allocates no material state; runtime pressure never changes exactness silently. |
| TEST-ARCH-003 | ARCH-007..009 | Architecture/roadmap gate | Proposed persistent view and post-PageRank algorithm | Build/reuse economics, format budget, spill invariants, receipt counters, buyer/oracle/artifact evidence, and paid next step are present; missing evidence blocks promotion. |
...
4. Generate and reconcile the frozen 384-file `docs_PRD03` through `docs_PRD06` denominator.
5. Fail while any of the nine algorithm families lacks three atlas options, a retained exact fit option, a retained exact spill option, or dual-corpus evidence.
6. Lint this specification for IDs, WHEN/THEN/SHALL clauses, test mappings, founder classifications, architecture registry references, and unresolved evidence placeholders.
...
4. Reconcile all 384 document evidence rows and validate the 27-option architecture atlas.
5. Freeze the first workload profile, option IDs, and oracle.
...
**RED:** Implement `TEST-ART-001..003`, `TEST-ADM-001..005`, `TEST-RCPT-001`, and `TEST-ARCH-001` against Rust traits and versioned schemas.
**GREEN:** Build artifact manifest validation, checked arithmetic, term-level estimator, versioned representation/option registry, deterministic selector, plan enum, and pre-run receipt with no algorithm execution.
...
**RED:** Implement `TEST-PLAN-003`, `TEST-ENF-001..004`, `TEST-RCPT-002..004`, and the forced-resource harness from `TEST-ARCH-002` with deliberately tiny ceilings and failure injection.
**GREEN:** Add shared reservations, supervised execution boundary, bounded buffers, versioned family spill invariants, spill files, cancellation, cleanup, progress events, and measured terminal receipts.
...
**GREEN:** Wire unmodified official Python driver -> Bolt -> bounded Cypher profile -> admission -> `ARCH-PATH-001` or `ARCH-PATH-002` or refuse -> exact path answer -> differential Neo4j oracle -> receipt.
...
## 3.6 Phase 4: WCC As The First Whole-Graph Proof

**RED:** Implement `TEST-ALG-004` and the WCC slice of `TEST-ARCH-002` over tiny partition truth, relabeling metamorphics, GDS differential, forced fit/spill/refuse, skewed components, and held-out estimator calibration.
**GREEN:** Add `ARCH-WCC-001` packed union-find and `ARCH-WCC-002` mapped-label/partitioned-edge spill behind the same equality and receipt contract.
**REFACTOR:** Extract only edge-block, label-plane, partition, and output primitives already proven reusable by the path slice.

## 3.7 Phase 5: PageRank As The First Iterative OLAP Proof
...
**GREEN:** Add GDS-shaped estimate/stream/stats/mutate/write behavior for the PageRank profile, `ARCH-PAGERANK-001` pull fit and `ARCH-PAGERANK-002` destination-tiled spill, deterministic output checksum, and preparation-versus-iteration receipts.
**REFACTOR:** Extract only storage/iteration primitives demonstrated reusable by WCC or another approved profile.

## 3.8 Phase 6: Earn Additional Algorithms

Implement similarity, community, triangles, kNN, and FastRP only in founder-evidence order and only after `TEST-ARCH-003` passes. Every addition repeats the full loop:
...
- `QG-EVID-008`: The document evidence union exactly matches all 384 frozen PRD03-PRD06 paths, hashes, byte counts, extensions, and lane assignments.
- `QG-EVID-009`: Every document with relevance `>= 80` is semantically read or structurally queried; generated/classified material is never represented as an independent semantic read.
- `QG-EVID-010`: Every retained architecture option cites at least one Neo4j-family source and one PRD/document source whose evidence ID exists in a reconciled ledger.

## 4.3 Architecture Gates

- `QG-ARCH-001`: Atlas validation passes with at least 27 options, all nine families, and a retained exact fit and exact spill option per family.
- `QG-ARCH-002`: Every admitted plan binds option, representation, estimator, selector, artifact, configuration, and calibration versions into the plan hash and receipt.
- `QG-ARCH-003`: Identical complete inputs select the same option; missing required statistics select a conservative alternate or refuse.
- `QG-ARCH-004`: Every advertised family passes forced fit, spill, and refusal tests under one declared equality relation before stable release.
- `QG-ARCH-005`: Mapped residency, output lower bounds, decoder/I/O windows, worker replicas, and temporary storage are explicit selection constraints.
- `QG-ARCH-006`: A persistent view is promoted only after first-run, repeat-run, amortized, disk, build-peak, invalidation, and measured-reuse evidence beats a simpler control.
- `QG-ARCH-007`: Paths, WCC, and PageRank close in order unless founder evidence explicitly changes the roadmap; later families require `TEST-ARCH-003`.
...
## 4.6 Latency and Benchmark Gates
...
## 4.7 Operational and Security Gates
...
1. Which captured customer artifact/query/resource ceiling freezes the first bounded path proof around `ARCH-PATH-001/002`?
2. At which graph/host thresholds should PageRank choose `ARCH-PAGERANK-001` pull fit versus `ARCH-PAGERANK-002` destination-tiled spill, and does `003` ever beat both after quality accounting?
3. For WCC, which measured shapes favor `ARCH-WCC-001` packed union-find, `002` mapped labels, or `003` hook-shortcut after pass/page-fault accounting?
...
The three repository evidence lanes pass `scripts/validate_neo4j_family_evidence.py`: 32,262 denominator rows equal 32,262 evidence rows with no identity, blob, byte, extension, status, relevance, critical-read, or evidence-ID violations. The three document lanes pass `scripts/validate_lowram_document_evidence.py`: 384 frozen files equal 384 evidence rows. The architecture atlas passes `scripts/validate_lowram_architecture_atlas.py`: 27 options across nine families with retained exact fit and spill choices. Repository/document coverage no longer blocks specification work.

Resolved architecture decisions:

1. Build the bounded path proof first, WCC second, and PageRank as the first iterative proof.
2. Use one small immutable artifact spine plus content-addressed algorithm-shaped views; do not build a database or one universal serving representation.
3. Require an exact fit and exact spill plan for every advertised family; approximation is additional and explicit.
4. Bind every run to a versioned atlas option and refuse when its full working-set, output, mapped-residency, temporary, time, or semantic contract cannot be honored.
5. Keep the remaining six families architecture-ready but roadmap-deferred until buyer evidence passes `REQ-ARCH-009.0`.

These decisions still require founder, implementation, measurement, or legal evidence:
