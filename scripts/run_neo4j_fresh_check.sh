...
load_runtime_paths_now() {
  ...
  FRESHCHECK_WORK_DIR="${KNIGHT_BUS_FRESHCHECK_WORK_DIR:-$ARTIFACTS_DIR/freshcheck_runtime}"
  ...
  export BENCH_VENV_DIR ARTIFACTS_DIR REPORTS_DIR FRESHCHECK_WORK_DIR BENCH_ENV_FILE INSTALL_SCRIPT REQUIREMENTS_FILE PYTHON_BIN
}
...
require_fixed_inputs_now() {
  local dataset_dir="$1"
  local required_paths=(
    "$dataset_dir/nodes.csv"
    "$dataset_dir/edges.csv"
    "$dataset_dir/query_corpus.csv"
    "$dataset_dir/manifest.json"
    "$dataset_dir/neo4j_import/nodes.header.csv"
    "$dataset_dir/neo4j_import/nodes.data.csv"
    "$dataset_dir/neo4j_import/relationships.header.csv"
    "$dataset_dir/neo4j_import/relationships.data.csv"
  )
  for required_path in "${required_paths[@]}"; do
    if [[ ! -f "$required_path" ]]; then
      echo "Missing fixed fresh-check input: $required_path" >&2
      exit 1
    fi
  done
}

import_neo4j_dataset_now() {
  local neo4j_import_dir="$1"
  local report_dir="$2"
...
run_single_stage_now() {
  local report_stage_name="$1"
  local source_stage_name="$2"
  local dataset_dir="$ARTIFACTS_DIR/$source_stage_name"
  local fresh_stage_dir="$FRESHCHECK_WORK_DIR/$report_stage_name"
  local snapshot_dir="$fresh_stage_dir/snapshot"
  local report_dir="$REPORTS_DIR/$report_stage_name"

  require_fixed_inputs_now "$dataset_dir"
  rm -rf "$fresh_stage_dir" "$report_dir"
  mkdir -p "$fresh_stage_dir" "$report_dir"

  "$KNIGHT_BUS_BIN_PATH" build \
    --nodes-csv "$dataset_dir/nodes.csv" \
    --edges-csv "$dataset_dir/edges.csv" \
    --output "$snapshot_dir" | tee "$report_dir/rust-build.txt"

  import_neo4j_dataset_now "$dataset_dir/neo4j_import" "$report_dir"

  "$BENCH_VENV_DIR/bin/python" "$REPO_ROOT/benchmarks/walk_hopper_v1/bench_walk_vs_neo4j.py" \
    --dataset "$dataset_dir" \
    --snapshot "$snapshot_dir" \
    --corpus "$dataset_dir/query_corpus.csv" \
...
main() {
  ...
  create_benchmark_venv_now
  build_knight_bus_binary_now
  mkdir -p "$REPORTS_DIR" "$FRESHCHECK_WORK_DIR"

  run_single_stage_now "freshcheck_neo4j_smoke_1mb" "neo4j_smoke_1mb"
  run_single_stage_now "freshcheck_neo4j_preflight_50mb" "neo4j_preflight_50mb"
  run_single_stage_now "freshcheck_code_sparse_2gb" "code_sparse_2gb"

  "$BENCH_VENV_DIR/bin/python" "$FINAL_JOURNAL_SCRIPT" \
    --repo-root "$REPO_ROOT" \
    --reports-dir "$REPORTS_DIR" \
    --final-journal "$REPO_ROOT/Final-Testing-Journal-v002.md" \
    --truth-journal "$REPO_ROOT/journal-tests-202604-v002.md" \
    --stage "freshcheck_neo4j_smoke_1mb=1 MB" \
    --stage "freshcheck_neo4j_preflight_50mb=50 MB" \
    --stage "freshcheck_code_sparse_2gb=2 GB"
}
