...
## Pattern 21: Driver Retry, Bookmark, And Routing Semantics

### Where Found

- Language/framework: Go, official Neo4j Go driver.
- Repo and files:
  - `neo4j-go-driver-src/neo4j/bookmarks.go`
  - `neo4j-go-driver-src/neo4j/session.go`
  - `neo4j-go-driver-src/neo4j/internal/retry/state.go`
  - `neo4j-go-driver-src/neo4j/internal/bolt/bolt4.go`
  - `neo4j-go-driver-src/neo4j/session_test.go`

### Evidence Tool Pass

CodeGraphContext was run on the focused repo:

`/tmp/codex-code-intel/codegraphcontext/neo4j-go-driver-src-20260706-234616`

Useful CGC evidence:

- stats: 219 files, 1,758 functions, 41 interfaces, 222 structs, 13 modules.
...
codebase-memory was also run:

`/tmp/codex-code-intel/codebase-memory/neo4j-go-driver-src-20260706-235049`

It indexed 3,034 nodes and 16,234 edges. `search_graph` for
`UpdateBookmarks` found the concrete method in `neo4j/bookmarks.go`.

### What The Code Does

The Go driver makes several client-visible Neo4j contracts explicit:

- `bookmarkManager` protects causal bookmarks with `sync.RWMutex`.
  `UpdateBookmarks` removes previous bookmarks, adds new bookmarks, and
  optionally notifies a consumer. `GetBookmarks` merges internal and supplied
  bookmark state.
- `ExecuteRead` and `ExecuteWrite` are thin API helpers over `runRetriable`,
  so retry behavior has one policy instead of being duplicated per caller.
- `retry.State` owns max retry duration, throttle, dead-connection budget,
  context cancellation, and retryability checks.
- `GetRoutingTable` varies by negotiated Bolt minor version: newer route
  messages, v4.3 route messages, or system-database Cypher fallback.
- Bolt pull/discard/commit success handlers update bookmarks from stream
  summaries or commit metadata.

### Why It Matters For Neo4j-In-Rust

A lower-RAM Rust rewrite can change storage layout and graph algorithm
execution, but it cannot casually change driver-visible behavior. Official
drivers encode expectations around retry, routing, bookmarks, stream
summaries, commit errors, and idempotent auto-commit retry.

The rewrite should treat this as a verification loop:

```text
driver helper API -> session retry loop -> connection pool -> Bolt FSM
                  -> routing table acquisition -> bookmark propagation
```

### Rust Translation

Represent retry as an explicit state object rather than scattered loops:

```rust
struct RetryState {
    errs: Vec<DriverError>,
    started_at: Instant,
    max_retry_time: Duration,
    dead_errors: usize,
    max_dead_connections: usize,
    telemetry_sent: bool,
    skip_sleep: bool,
}
```

Represent bookmarks as isolated causal state:

```rust
struct BookmarkManager {
    bookmarks: Arc<RwLock<IndexSet<Bookmark>>>,
}
```

The implementation detail can change, but the tests should assert the same
observable contract.

### Executable Specs To Preserve Compatibility

```text
WHEN ExecuteWrite receives a retryable transient database error
THEN it SHALL retry until success or retry budget exhaustion
AND it SHALL return each failed attempt connection to the pool.

WHEN ExecuteWrite receives a user error or commit-failed-dead error
THEN it SHALL NOT retry
AND the session SHALL return to clean state.

WHEN a Bolt connection retrieves a routing table
THEN it SHALL choose the routing-table mechanism by negotiated Bolt version.

WHEN stream summary or commit success contains a bookmark
THEN the connection/session SHALL update causal bookmark state.
```

### How Future Agents Should Apply It

Do not start by implementing driver semantics inside storage code. Build a
small compatibility test harness around session retry, routing, and bookmarks,
then rewrite storage/query internals behind that harness.
...
| `neo4j-go-driver-src` | Medium | CGC, codebase-memory, and direct source reads | More value encoding and Testkit backend details |
