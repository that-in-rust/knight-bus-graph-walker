# Supplemental Storage And Rust Systems Patterns

This supplemental file preserves an earlier storage-heavy Worker 3 encyclopedia
slice for rewriting a Neo4j-like graph store in Rust with lower RAM usage. The
canonical storage slice is now `supermeta-graph-database-patterns-2.md`; this
supplement remains useful because it contains additional source-backed material
on page/cache layout, mmap and zero-copy reads, WAL/checkpoint recovery,
indexes, allocators, unsafe encapsulation, concurrency, and verification.
